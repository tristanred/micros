...
int vsprintf( char *buffer, const char *format, va_list vlist )
{
    ...
    while(go == 0)
    {
        ...
        else if(format[sourceIndex] == '%' && format[sourceIndex + 1] == 'b')
        {
            sourceIndex += 2;

            // Custom Binary format string
            // If the parameter compares to TRUE (not zero) outputs 1
            // If the parameter compares to FALSE (zero) outputs 0
            uint32_t value = va_arg(vlist, uint32_t);
            if(value > 0)
            {
